...
namespace RDMAUtil {
        ...
        -> StatusPair
    {
        ...
        auto byte_buf = reinterpret_cast<uint8_t *>(buf) + local_offset;
        ...
        sr.opcode     = opcode;
        sr.next = NULL;
        if(opcode == IBV_WR_RDMA_WRITE && msg_len <= MAX_INLINE_SIZE)
    ...
    auto RDMAContext::post_send_helper(const uint8_t * &ptr, const uint8_t *msg,
    ...
    auto RDMAContext::post_send(const uint8_t * &ptr, uint8_t *msg, size_t msg_len,
                                size_t local_offset)
        -> StatusPair
    {
    ...
    auto RDMAContext::post_read(const uint8_t * &ptr, size_t msg_len, size_t local_offset)
        -> StatusPair
    {
    ...
    auto RDMAContext::post_write(const uint8_t * &ptr, const uint8_t *msg, size_t msg_len,
                                 size_t local_offset)
        -> StatusPair
    {
    ...
    auto RDMAContext::generate_sge(const uint8_t * msg, size_t msg_len, size_t offset)
    ...
    auto RDMAContext::generate_send_wr(int wr_id, struct ibv_sge *sge, int num_sge, const uint8_t * remote,
    ...
    auto RDMAContext::post_batch_write(struct ibv_send_wr *wrs) -> StatusPair {
        ...
        if (auto ret = ibv_post_send(qp, wrs, &bad_wr); ret != 0) {
            fprintf(stderr, "posting wr %ld failed, error code: %d\n", bad_wr->wr_id, ret);
    ...
    auto RDMAContext::post_batch_write_test() -> void {
        ...
        auto wr1 = generate_send_wr(0, sge1.get(), 1, (uint8_t *)remote.addr, nullptr);
        auto wr2 = generate_send_wr(1, sge2.get(), 1, (uint8_t *)remote.addr + 8, nullptr);
        wr1->next = wr2.get();
        wr1->send_flags = 0;
        ...
        if (auto ret = ibv_post_send(qp, wr1.get(), &bad_wr); ret != 0) {
            fprintf(stderr, "posting wr %ld failed\n", bad_wr->wr_id);
            return;
        }

        auto ret = poll_one_completion();
    ...
    auto RDMAContext::poll_one_completion(bool send) noexcept -> int {
        struct ibv_wc wc;
        auto cq = send ? out_cq : in_cq;
        int ret;
        do {
    ...
    auto RDMAContext::fill_buf(const uint8_t *msg, size_t msg_len, size_t offset)
        -> uint8_t *
    {
        if (msg && msg_len != 0)
    ...
    const struct ibv_qp_init_attr RDMADevice::ZATTR = {0};
    ...
    {   
        if(mr_access == 0) {
            mr_access = get_default_mr_access();
        }

        if(memcmp(&attr, &ZATTR, sizeof(struct ibv_qp_init_attr)) == 0) {
            attr = get_default_qp_init_attr();
        }
        ...
        rdma_ctx->buf = membuf;
        rdma_ctx->bufsize = memsize;
        rdma_ctx->dmabuf = false;
        rdma_ctx->device = this;
        return {std::move(rdma_ctx), Status::Ok};
    }

    auto RDMADevice::open(int dmabuf_fd, size_t buflen, size_t cqe, int mr_access,
                          struct ibv_qp_init_attr attr)
        -> std::pair<std::unique_ptr<RDMAContext>, Status> {
        if(mr_access == 0) {
            mr_access = get_default_mr_access();
        ...
        if (!(rdma_ctx->mr = ibv_reg_dmabuf_mr(rdma_ctx->pd, 0, buflen, 0, dmabuf_fd, mr_access))) {
            return {nullptr, Status::CannotRegMR};
        }
        ...
        rdma_ctx->local.addr = (uint64_t)rdma_ctx->mr->addr;
        rdma_ctx->local.rkey = rdma_ctx->mr->rkey;
        rdma_ctx->local.qp_num = rdma_ctx->qp->qp_num;
        ...
        rdma_ctx->buf = mmap(0, buflen, PROT_READ | PROT_WRITE, MAP_SHARED, dmabuf_fd, 0);
        rdma_ctx->bufsize = buflen;
        rdma_ctx->dmabuf = true;
        rdma_ctx->device = this;
        return {std::move(rdma_ctx), Status::Ok};
    }
    ...
    auto RDMADevice::get_default_qp_init_attr() -> struct ibv_qp_init_attr
    {
        ...
        attr.sq_sig_all = 0;
        attr.cap.max_send_wr = Constants::MAX_QP_DEPTH;
        attr.cap.max_recv_wr = Constants::MAX_QP_DEPTH;
