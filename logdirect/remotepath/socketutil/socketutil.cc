...
std::unique_ptr<Socket> Socket::make_socket(SockType s, int port, std::string ipaddr) {
    ...
    if(new_socket->socktype == SERVER) {
        ...
        if ((new_socket->commu_sock = accept(new_socket->listen_sock, (struct sockaddr*)&address, (socklen_t*)&addrlen)) < 0) {
            perror("accept");
            exit(EXIT_FAILURE);
        }
    } else {
        ...
        new_socket->commu_sock = init_sock;
        ...
        // connect to a server
        if (connect(new_socket->commu_sock, (struct sockaddr*)&address, sizeof(address)) < 0) {
            perror("connect");
            exit(EXIT_FAILURE);
        }
    }
